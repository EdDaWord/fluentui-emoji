...
function buildUnicodeIndexFromCategories(categoriesEmojis) {
  ...
  function normalizeCodeArray(codeArr) {
    // codeArr is like ["1F600"] or ["1F469","200D","1F4BB"]
...
async function processGoogleAssets(options = {}) {
  ...
  console.log('Creating manualGoogle directories...');
  ...
  for (const filename of pngFiles) {
    try {
      // Convert Google filename to normalized unicode key for index
      // emoji_u1f60a.png or emoji_u1f469_200d_1f4bb.png -> 1f60a or 1f469200d1f4bb
      ...
      if (!googleCategoryStats[category]) {
        googleCategoryStats[category] = 0;
        googleCategoryEntries[category] = [];
      }
      ...
      try {
        ...
        // Create JSONL entries for google category
        for (const caption of captions) {
          const googleRelativePath = `data/${category}/${safeFilename}`;
          googleCategoryEntries[category].push({
            image: googleRelativePath,
            caption: caption
          });
        }
        
        googleCategoryStats[category]++;
        processedCount++;
  ...
  let totalEntries = 0;
  for (const [category, entries] of Object.entries(googleCategoryEntries)) {
    if (entries.length > 0) {
      ...
      const camelCaseName = category
        ...
        .join('');
      const outputFile = path.join(googleJsonlDir, `${camelCaseName}.jsonl`);
  ...
  console.log(`\nGoogle dataset generation complete!`);
  console.log(`- Processed: ${processedCount} assets`);
  console.log(`- Skipped: ${skippedCount} assets`);
  console.log(`- Total JSONL entries: ${totalEntries}`);
  console.log(`- Images copied to: ${googleDataDir}`);
  console.log(`- JSONL files written to: ${googleJsonlDir}`);
  console.log(`\nGoogle category breakdown:`);
  for (const [category, count] of Object.entries(googleCategoryStats)) {
